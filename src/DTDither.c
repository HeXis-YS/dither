...
void ApplyFloydSteinbergDither(DTImage *image, DTPalette *palette) {
    for (int i = 0; i < image->height; i++) {
        for (int j = 0; j < image->width; j++) {
            ...
            // disperse
            if (j + 1 < image->width) {
                ApplyDifference((image->pixels + i * image->width + j + 1), diff, 7);
            }
            if (i + 1 < image->height) {
                if (j - 1) {
                    ApplyDifference((image->pixels + (i + 1) * image->width + j - 1), diff, 3);
                }
                ApplyDifference((image->pixels + (i + 1) * image->width + j), diff, 5);
                if (j + 1 < image->width) {
                    ApplyDifference((image->pixels + (i + 1) * image->width + j + 1), diff, 1);
                }
            }
